...
- Language Model: Generates answers using `Qwen/Qwen2.5-1.5B-Instruct`,
  a small, free, instruction-tuned model that runs entirely on the
  local machine through the transformers library. No external API,
  account, or token is required. Responses are grounded in retrieved
...
- Sufficient local CPU and memory to run a 1.5B parameter language
  model. 8 GB of available RAM or more is recommended.
